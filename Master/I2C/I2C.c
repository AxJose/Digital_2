...
uint8_t I2C_Master_Read_Ack(void) {
	// Habilitar TWI para recibir un byte y responder con ACK
	TWCR = (1 << TWINT) | (1 << TWEN) | (1 << TWEA);
	while (!(TWCR & (1 << TWINT))); // Esperar la recepción
	return TWDR;
}

uint8_t I2C_Master_Read_Nack(void) {
	// Habilitar TWI para recibir un byte y responder con NACK
	TWCR = (1 << TWINT) | (1 << TWEN);
	while (!(TWCR & (1 << TWINT))); // Esperar la recepción
	return TWDR;
}
